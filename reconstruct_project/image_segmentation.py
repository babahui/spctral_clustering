...
def large_image_seg(img_name="train/35070", sp_num=1000, sp_mode="similarity", sp_connectivity=2):
    # read image and save as a matrix
    base_name = "/home/yy/matlab/BSR/BSDS500/data/images/"
    ...
    sp_label = admm_algorithms.admm(n_vector=ev2, num_cuts=10)

    image_plot(sp_label, segments, m_img)
...
def superpixels_label_test(m_img, segments, sp_mode="similarity", sp_connectivity=2, num_cuts=5):
    image = m_img
    ...
    for i in range(length):
        for j in range(length):
            ...
            diff = abs(red_average[i]-red_average[j]) ** 2 + abs(green_average[i]-green_average[j]) ** 2 + abs(blue_average[i]-blue_average[j]) ** 2
