...
def initDatasets():
    ...
    for i in GauSize:
        ...
        if i == 1:
            ...
            X1 = np.append(X1, x1)
            X2 = np.append(X2, x2)
            y = np.dot(i, np.ones(5))
            Y = np.append(Y, y)
        if i == 2:
            x1, x2 = np.random.multivariate_normal(mean, cov, 15).T
            X1 = np.append(X1, x1)
            X2 = np.append(X2, x2)
            y = np.dot(i, np.ones(15))
            Y = np.append(Y, y)
        if i == 3:
            x1, x2 = np.random.multivariate_normal(mean, cov, 10).T
            X1 = np.append(X1, x1)
            X2 = np.append(X2, x2)
            y = np.dot(i, np.ones(10))
            Y = np.append(Y, y)
        mean2 = [20, 35]
        if i == 4:
            x1, x2 = np.random.multivariate_normal(mean2, cov, 10).T
            X1 = np.append(X1, x1)
            X2 = np.append(X2, x2)
            y = np.dot(i, np.ones(10))
            Y = np.append(Y, y)
        mean3 = [30, 35]
        if i == 5:
            x1, x2 = np.random.multivariate_normal(mean3, cov, 10).T
            X1 = np.append(X1, x1)
            X2 = np.append(X2, x2)
            y = np.dot(i, np.ones(10))
            Y = np.append(Y, y)
